...
test('local manager configures, unlocks, issues, records, and backs up', async () => {
  ...
  try {
    ...
    assert.deepEqual(setup.app.majorVersions, [1])
...
test('new machine restores a complete App from one encrypted backup', async () => {
  ...
  try {
    ...
    const created = await call('create-app', { appId: 'restored_app', name: 'Restored App', majorVersion: 99, kid: 'key-2', password: 'manager-test-password', backupDirectory: external })
    assert.deepEqual(created.app.majorVersions, [1])
    const released = await call('release-major-version', { appId: 'restored_app' })
    assert.deepEqual(released.app.majorVersions, [1, 2])
  ...
  try {
    ...
    assert.equal(value.app.name, 'Restored App'); assert.equal(value.app.majorVersion, 2); assert.deepEqual(value.app.majorVersions, [1, 2]); assert.equal(value.app.records.length, 1); assert.deepEqual(value.app.plans, ['free'])
...
test('device-bound App requires a matching device request when issuing', async () => {
  ...
  try {
    ...
    const released = await call('release-major-version', { appId: 'bound_app' }); assert.equal(released.response.ok, true)
    ...
    const wrongMajor = await call('issue', { appId: 'bound_app', majorVersion: 2, plan: 'pro', deviceRequest: request })
    assert.equal(wrongMajor.response.ok, false)
    const issued = await call('issue', { appId: 'bound_app', majorVersion: 1, plan: 'pro', deviceRequest: request })
    assert.equal(issued.response.ok, true); assert.equal(issued.value.issued.payload.deviceId, 'device-123'); assert.equal(issued.value.issued.payload.majorVersion, 1)
  } finally { await new Promise<void>(resolve => manager.server.close(() => resolve())) }
})
