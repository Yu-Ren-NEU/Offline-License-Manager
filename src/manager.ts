...
(()=>{
  ...
  function setupPlanCatalog(){
    ...
    $('plan').innerHTML='<option value="">Choose a Plan</option>'+app.plans.map(plan=>'<option value="'+esc(plan)+'">'+esc(plan)+'</option>').join('')
    if(app.plans.includes(current))$('plan').value=current
  }
  function setupDeviceAutofill(){
    ...
    field.oninput=()=>{
      ...
      if(latestPlan)$('plan').value=latestPlan
