...
type BackupContent = { version: 1; appId: string; kid: string; createdAt: string; encryptedPrivateKey: unknown; publicKey: unknown; records: IssuedLicenseRecord[] }
...
export async function createBackup(options: { appId: string; encryptedKeyFile: string; publicKeyFile: string; recordsFile: string; outputFile: string; password: string }) {
  ...
  const content: BackupContent = { version: 1, appId: options.appId, kid: publicKey.kid, createdAt: new Date().toISOString(), encryptedPrivateKey, publicKey, records: await readIssuedLicenseRecords(options.recordsFile) }
